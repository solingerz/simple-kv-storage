...
- **公开 / 私有**：上传后默认**私有**，可随时切换；私有文件 URL 立即失效（返回 403）
- **直链复制**：文件设为公开后，一键复制可分享的直链
- **鉴权保护**：除公开文件下载外，所有操作均需 HTTP Basic Auth
- **不可预测 URL**：文件链接格式为 `domain.com/f/{32位随机hex}/{文件名.后缀}`
...
| Key | Value | 说明 |
|---|---|---|
| `meta:{hash}` | JSON 字符串 | 文件元数据 |
| `file:{hash}` | ArrayBuffer | 文件原始内容 |
...
| 总存储容量 | 1 GB |
| KV 命名空间数 | 1 个（绑定为 `KV`）|
| 文件 URL 格式 | `/f/{32位随机hex}/{原始文件名}` |
| 上传后默认可见性 | 私有 |

## 可见性说明

- **私有（默认）**：文件 URL 返回 `403 Forbidden`，无法被任何人访问，无法复制分享链接
- **公开**：文件 URL 可直接访问和分享，支持 CDN 长期缓存

切换为私有后，**原有的公开链接立即失效**，无需手动处理缓存或 URL。
